...
(function(factory) {
	...
	$.extend($.validator, {
		...
		messages: {
			...
			url: "请检查地址",
			date: "请输入正确的日期",
			dateISO: "请检查日期格式：YYYY-mm-dd",
			number: "请输入数字",
			digits: "只能输入自然数",
			creditcard: "请输入有效的信用卡",
			equalTo: "两次属于不一致",
			maxlength: $.validator.format("字符串长度最大{0}位"),
			minlength: $.validator.format("字符串长度小于{0}位"),
			rangelength: $.validator.format("字符串长度在{0}到{1}位之间"),
			range: $.validator.format("数值在{0}和{1}之间"),
		...
		methods: {
			...
			idcard: function(value, element) {
				...
				if(value.length == 18) valid = /^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X|x)$/; //18位
			...
			money: function(value, element) {
				return this.optional(element) || /^((^0\.\d{0,2}$)|(^[1-9]+\d*\.\d{0,2}$)|(^[1-9]\d*$)|(^0$))?$/.test(value);
			},
			//邮编
			...
			password: function(value, element) {
				return this.optional(element) || /^(?![a-z]+$)(?!\d+$)[a-z0-9]{6,20}$/i.test(value);
			},
			//中文
			...
			//正浮点数
			pfloat: function(value, element) {
				return this.optional(element) || /^[1-9]\d*\.\d*|0\.\d*[1-9]\d*$/.test(value);
			},
			//负浮点数
			nfloat: function(value, element) {
				return this.optional(element) || /^-[1-9]\d*\.\d*|-0\.\d*[1-9]\d*$/.test(value);
			},
			//正整数
			pInt: function(value, element) {
				return this.optional(element) || /^[1-9]\d*$/.test(value);
			},
			//负整数
			nInt: function(value, element) {
				return this.optional(element) || /^-[1-9]\d*$/.test(value);
			}
		}
